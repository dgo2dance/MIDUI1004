	...
	$.fn.zyUpload = function(options,param){
		...
		return this.each(function(){
			...
			var defaults = {
					...
					url              : "<%=basePath%>/userbasic/photoLoadSave.do",  	// 上传文件的路径
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
		            ...
		            //html += '			<div class="status_bar">';
		            //html += '				<div id="status_info" class="info">选中0张文件，共0B。</div>';  //accept="image/jpeg, image/png, image/jpg"
		           // html += '				<div class="btns">';
		            html += '					<input id="fileImage" type="file" size="30" accept="image/jpeg, image/png, image/jpg"  name="fileselect[]" '+multiple+'>';
		           ...
		           // html += '			</div>';
				    ...
				    html += '							<img class="upload_image" src="../midui/ImgLoad/control/images/add_img.png" style="width:expression(this.width > '+imgWidth+' ? '+imgWidth+'px : this.width)" />';
				    html += '						</div>';
				    html += '					</a>';
				    html += '				</div>';
					html += '			</div>';
